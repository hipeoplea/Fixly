...
В отличие от универсальных платформ вроде Авито или Юлы, где аренда инструментов не выделена в отдельное направление и отсутствуют встроенные механизмы залога и защиты пользователей в объявлениях такого формата,
сервис Fixly ориентирован именно на безопасную p2p-аренду. Это позволяет упростить поиск и оформление сделок, добавить систему отзывов и рейтингов, автоматизировать контроль сроков и условий аренды.
В результате Fixly может предложить пользователям более специализированный и надёжный сервис по сравнению с крупными, но общими по тематике конкурентами.
...
*Требования к производительности*\
    #p(1, [Платформа должна поддерживать до 1000 одновременных пользователей при нагрузочном тестировании без существенного падения производительности (95-й процентиль времени отклика основных операций ≤ 5 секунд в течение 15 минут теста)])\
    #p(2, [95-й процентиль времени отклика на запросы поиска не должен превышать 3 секунд при тестовой нагрузке в течение 1 часа])\
    #p(3, [95-й процентиль времени загрузки страницы с результатами поиска за любой интервал в 1 час не должен превышать 2 секунд])\
    #p(4, [95-й процентиль времени отклика при оформлении аренды не должен превышать 5 секунд в течение 30 минут тестовой нагрузки])
...
- *Требование к безопасности и поддерживаемости*\
    ...
    #s(6, [Все API и фронтенд-авторизация должны работать через протокол OAuth2])\
    #s(7, [Внутренние сервисы и компоненты взаимодействуют только по защищённым каналам (TLS) с обязательной валидацией сертификатов])
...
Реализуется с использованием Prometheus (сбор метрик), Grafana (дашборды и алерты), Actuator в Spring Boot для экспорта метрик (/actuator/prometheus), с базовыми дашбордами по RPS, латентности, ошибкам, ресурсам JVM и алертами на 5xx, высокую задержку и нехватку ресурсов.
